...
export class CharacterController {
  ...
  async createCharacter(@Body() createCharacterDto: CreateCharacterDto) {
    ...
    if (
      !Object.values(Specialization).includes(
        createCharacterDto.specialization as Specialization,
      )
    ) {
  ...
  async upsertCharacter(@Body() body: any, @Req() req: any) {
    ...
    const upsertCharacterDto: UpsertCharacterDto = {
      id:
        body.id !== undefined && body.id !== null ? Number(body.id) : undefined,
      name: body.name,
      characterClass: body.characterClass,
      specialization: body.specialization,
      iLevel:
        body.iLevel !== undefined && body.iLevel !== null
          ? Number(body.iLevel)
          : undefined,
      eventCode: body.eventCode,
      keystoneMinLevel:
        body.keystoneMinLevel !== undefined && body.keystoneMinLevel !== null
          ? Number(body.keystoneMinLevel)
          : undefined,
      keystoneMaxLevel:
        body.keystoneMaxLevel !== undefined && body.keystoneMaxLevel !== null
          ? Number(body.keystoneMaxLevel)
          : undefined,
    };

    // ✅ Validation de la spécialisation (comme Express ligne 117-119)
    if (
      upsertCharacterDto.specialization &&
      !Object.values(Specialization).includes(
        upsertCharacterDto.specialization as Specialization,
      )
    ) {
      throw new HttpException('Invalid specialization', HttpStatus.BAD_REQUEST);
    }
    ...
    const updatedCharacter =
      await this.characterService.upsertCharacter(upsertCharacterDto);

    if (!updatedCharacter) {
      throw new HttpException(
        'Error upserting character',
  ...
  async updateCharacter(
    ...
    if (
      updateCharacterDto.specialization &&
      !Object.values(Specialization).includes(
        updateCharacterDto.specialization as Specialization,
      )
    ) {
      throw new HttpException('Invalid specialization', HttpStatus.BAD_REQUEST);
    }
