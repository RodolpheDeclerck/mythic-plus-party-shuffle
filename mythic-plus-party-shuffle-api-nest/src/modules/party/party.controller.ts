...
export class PartyController {
  ...
  async createOrUpdateParties(
    ...
    this.logger.debug(
      `[createOrUpdateParties] raw body: ${JSON.stringify(body)}`,
    );
    ...
    try {
      ...
      const transformedParties = parties.map((party, partyIndex) => {
        if (!party.members || !Array.isArray(party.members)) {
          throw new Error(
            `Party at index ${partyIndex} must have a members array`,
          );
        }
        ...
        return {
          ...
          members: party.members.map((member: any, memberIndex: number) => {
            ...
            if (typeof member === 'object' && member !== null) {
              ...
              throw new Error(
                `Member at party ${partyIndex}, member ${memberIndex} must have an id property`,
              );
            }
            throw new Error(
              `Invalid member format at party ${partyIndex}, member ${memberIndex}: ${typeof member}`,
            );
          }),
        };
      });
      ...
      this.logger.debug(
        `[createOrUpdateParties] transformed parties: ${JSON.stringify(transformedParties)}`,
      );
      ...
      const partiesFromRedis =
        await this.partyService.getPartiesByEventCode(eventCode);
      this.webSocketService.emitPartiesUpdated(partiesFromRedis);

      this.logger.log(
        `[createOrUpdateParties] updated parties for event ${eventCode}`,
      );
